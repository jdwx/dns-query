...
class ResourceRecord implements ResourceRecordInterface, ArrayAccess {
    ...
    public function __construct(
        ...
        if ( ! isset( RDataMaps::MAP_LIST[ $this->type->value ] ) ) {
            throw new \InvalidArgumentException( "Unsupported record type: {$this->type->name}" );
        }
        $this->rDataMap = RDataMaps::MAP_LIST[ $this->type->value ];
    ...
    public static function fromString( string $i_string ) : self {
        ...
        $rData = [];
        foreach ( RDataMaps::MAP_LIST[ $type->value ] as $stName => $rdt ) {
            if ( empty( $r ) ) {
